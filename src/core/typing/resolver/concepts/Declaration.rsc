module core::typing::resolver::concepts::Declaration
...
&T <: node checkReturnType( &T <: node n, Type expectedReturnType ) {
	...
	if( !( isEmpty( nType ) ) && !inTypeTree( CTypeTree, getType( expectedReturnType ), nType ) ) {
...
Decl resolve( Decl f:function(_, _, _, _, _) ) {
	<f,_,stats> = resolveFunctionReturnType( f, f.stats, resolveTypeDefs( f@indextable, f.\type ) );
