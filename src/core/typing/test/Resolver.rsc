...
public test bool testAllowAssignmentOfPointerReferenceField() {
	str testCaseName = "testAllowAssignmentOfPointerReferenceField";
	if( PRINT ) { println("RUNNING: <testCaseName>"); }
	passed = true;
	str input = "module Test;
	typedef struct TrackPoint as TrackPoint;
	...
	TrackPoint* resetTP( TrackPoint* tp ) {
		tp-\>speed = 0;
		tp-\>alt = 0;
		return tp;
	}
